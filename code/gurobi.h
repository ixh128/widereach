...
/** Run gurobi relaxation only
 ...
 the zeroth element is the objective value in the relaxation solution at the end
 of the run, and the next elements are the values of the decision variables. */
double *gurobi_relax(
...
typedef struct sys7_t {
  gsl_matrix *A;
  gsl_vector *b;
  int *ref; // ref[i] is the index in env->samples corresponding to row i in
            // matrix A
} sys7_t;
...
GRBmodel *gurobi_milp_tgm(int *, const env_t *);
