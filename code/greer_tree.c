...
void compute_res_obj(env_t *env, class_res *res) {
  ...
  if(params->greer_params.obj_code == WRC) {
    double violation = (params->theta - 1)*res->ntpos + params->theta*res->nfpos;
    ...
    double min_viol = (params->theta - 1)*res->ntpos + params->theta*res->nfpos;
    if(min_viol < 0)
    ...
    int max_ntpos = res->ntpos + res->npz;
    res->max_poss_obj = max_ntpos / ((double) max_ntpos + res->nfpos); //every false positive will remain so, so min_nfpos = nfpos
  } else {
...
tnode *free_subtree(tnode *root) {
  ...
  //assert(!root->in_q); //this no longer works as a check, since root may always be in the heapq if we're using a stack
  tnode *next;
  while(root->l_child) {
...
gsl_vector *obtain_null_vec(gsl_matrix *A) {
  ...
  //final d-k columns of Q is an ONB for the null space of A^T, assuming A had full rank
...
gsl_matrix *get_parent_matrix(env_t *env, tnode *node) {
  ...
  if(env->params->greer_params.method == 2) {
    ...
    gsl_matrix_memcpy(stored, node->parent_basis); //need to copy, since the qr fac may destroy it
    return stored;
  }
...
double compute_bound(env_t *env, tnode *node) {
  int sz = node->depth; //maybe +/-1? should be correct, assuming root has depth 0
  ...
  gurobi_param p = {
	      ...
	      .init = NULL,
	      .cone = NULL,
	      .ortho = {.basis = basis, .n = sz},
  };

  double *h = single_gurobi_run(0, 6000, 0, env, &p);
  double obj = h[0];
  free(h);
  free(basis);
  return obj;
...
void create_node_child(env_t *env, tnode *node, sample_locator_t loc, int **branching_data, double score(env_t *, tnode *)) {
  ...
  //need to compute the classification result here in order to compute the score
  //unless we're using cuda, in which case that gets done later
...
double zero_score(env_t *env, tnode *node) {
  return 0;
}
...
double dist_penalty(env_t *env, tnode *node) {
  //returns sum of angles from node's hplane to all other known feasible solutions
  double sum = 0;
...
double compute_score(env_t *env, tnode *node) {
  ...
  if (env->params->greer_params.method == 3) {
    return node->data->res.obj + env->params->greer_params.gamma * dist_penalty(env, node);
  }
...
void add_children_to_stack_scored(tnodestack *s, tnode *node) {
  ...
  double *arr = CALLOC(node->n_children, double); //TODO - changed this int -> double without testing
  int k = 0;
  tnode *curr = node->l_child;
  while(curr) {
    arr[k] = curr->score;
...
void add_children_to_stack_scored_trunc(tnodestack *s, tnode *node, int n) {
  //adds the best n of node's children to s, in decreasing order of their scores
  tnode **children = CALLOC(node->n_children, tnode *);
  int k = 0;
  tnode *curr = node->l_child;
  while(curr) {
...
void add_children_to_heapq(heapq_t *pq, tnode *node) {
  ...
  tnode *curr = node->l_child;
  while(curr) {
    tnode *next = curr->r_sib; //need to do this now, since enqueue can free curr
...
void gr_update_b(hplane_list *B, hplane_data *vd) {
  ...
  //this doesn't match exactly what greer says, because his notation doesn't make sense
  if(DEBUG_GR)
    if(max_v_obj > 0) 
      printf("max obj = %d\n", max_v_obj);
...
int dim_zeros(vsamples_t *vs, hplane_data *vd) {
  ...
  //I'll do this by making them the columns of a matrix, then computing its rank via SVD
  ...
  if(ncols > d) {
    //gsl doesn't calculate SVD in this case, so we need to transpose the matrix
...
void update_a_displace(env_t *env, hplane_data *vd, gsl_vector *y, hplane_list *A) {
  ...
  hplane_data *displaced_data = make_hplane_data(displaced, classify(env, displaced));
  displaced_data->extra = 0;

  //printf("update_a_displace info:\n#zeros = %d\nv = ", vd->res.npz + vd->res.nnz); print_vec(vd->v); printf("y = "); print_vec(y); printf("v obj = %g\n", vd->res.obj); printf("y obj = %g\n", classify(env, y).obj); printf("resulting obj = %g\n", displaced_data->res.obj);
...
vsamples_t *construct_subvs(vsamples_t *vs, hplane_data *vd) {
  ...
  //note that it doesn't copy the sample vectors, so it should be freed using the below function
...
void solve_npc(env_t *env, hplane_data *vd, hplane_list *A, int dim) {
  ...
  env->vsamples = nvs; //TODO: change this to make a new env for the recursive call
...
void solve_highdim(env_t *env, hplane_data *vd, hplane_list *A, int dim) {
  ...
  case 1:
    ...
    break;
  case 0:
    if(env->params->greer_params.skip_rec) break;
...
void check_free(tnode *node, int trunc) {
  ...
  //to do this, it sets the variables children_explored in some of its ancestors
  ...
  while(curr->parent &&
	(curr->parent->children_explored >= ((trunc == 0) ? curr->parent->n_children : trunc)) &&
	curr->parent->depth > 0) {
    curr = curr->parent;
    //printf("moved back to node %d, which has %d children explored of %d total\n", curr->key, curr->children_explored, curr->n_children);
    //printf("  counted %d children\n", print_children(curr));
    //at this point, we know that curr has been fully explored
    //so we can update its parent
    curr->parent->children_explored++;
  }

  if(DEBUG_GR) printf("about to free subtree starting at node %d, depth %d\n", curr->key, curr->depth);
...
hplane_list *gr_explore(env_t *env, gsl_vector *v0, hplane_list *A) {
  ...
  while(get_size(coll) > 0) {
    ...
    tnode *curr = pop(coll);
    if(DEBUG_GR) printf("popped from stack, id = %d, score = %g\n", curr->key, curr->score);

    /*double dupe_arr[] = {-0.164399, -5.55112e-17, 2.99101e-14, -9.24781e-15, -1.93075e-15, -2.72005e-15, 0.986394};
    gsl_vector dupe = gsl_vector_view_array(dupe_arr, d).vector;
    if(dot(curr->data->v, &dupe) > 1-1e-8) {
      printf("found duplicate, path = ");
      tnode *trav = curr;
      while(trav) {
    ...
    nodes_explored++;
    if(time(0) - start > 0) {
      //printf("%d nodes explored, %d items in stack, %d nodes in tree, best obj = %g\n", nodes_explored, s->top, tree_nodes, best_obj);
      //printf("%d nodes explored, %ld items in pq, %d nodes in tree, best obj = %g\n", nodes_explored, pq->n, tree_nodes, best_obj);
      printf("%d nodes explored, %ld items in queue, %d nodes in tree, best obj = %g\n", nodes_explored, get_size(coll), tree_nodes, best_obj);
      start = time(0);
    }
    ...
    /*if(res.nfneg + res.nfpos + res.nnz + res.npz - curr->depth < root.data->res.nfneg + root.data->res.nfpos && !trunc) {
      ...
      free_subtree(&root);
      printf("restarting, new solution has at most %d errors (A has %d hplane(s))\n", res.nfneg + res.nfpos + res.nnz + res.npz - curr->depth, A->count);
      return gr_explore(vs, new_v0, A, trunc);
      }*/
...
gsl_vector *max_accuracy_random_hyperplane(env_t *env, int N) {
  //returns the best of N random hyperplanes, where "best" means the one which has the fewest misclassifications
  vsamples_t *vs = env->vsamples;
  size_t d = vs->dimension;
...
gsl_vector *successive_trunc(env_t *env, gsl_vector *v0, int start, int inc, int max) {
...
void copy_parent_basis(env_t *env, tnode *node) {
  //alloc's node->parent_samples, copies it from parent, and copies parent's sample into it
  if(node->depth == 0) return;
  if(node->parent_basis) return; //if basis already exists, no need to make a new one
  vsamples_t *vs = env->vsamples;
  size_t d = vs->dimension;
  ...
  for(int i = 0; i < depth-1; i++) {
    gsl_vector col = gsl_matrix_const_column(node->parent->parent_basis, i).vector;
...
void score_all_from_mat(env_t *env, gsl_matrix *mat, tnode **children, int n_children) {
  //mat is matrix SW of dot product results
  //this is O(mn), so it'd be great to do on GPU, but difficult to work with the data strctures
  ...
  for(int j = 0; j < mat->size2; j++) {
    ...
    children[j]->data->res.nwrong = children[j]->data->res.nfneg + children[j]->data->res.nfpos;
    children[j]->data->res.nright = children[j]->data->res.ntpos + children[j]->data->res.ntneg;

    compute_res_obj(env, &children[j]->data->res);
    children[j]->score = compute_score(env, children[j]);
    //printf("computed score %g for node %d\n",     children[j]->score, children[j]->key);
  }
}
...
int beam_iter(env_t *env, tnode **beam, size_t width) {
  ...
  if(env->params->greer_params.classify_cuda) {
    ...
    for(int i = 0; i < n_children-batch_size; i += batch_size) {
      gsl_matrix *mat = classify_many(env, children+i, fmin(n_children - i, batch_size));
      printf("matrix has shape (%ld, %ld)\n", mat->size1, mat->size2);
...
hplane_list *beam_search(env_t *env, gsl_vector *v0, hplane_list *A) {
  ...
  int done = 0;
  while(!done) {
    if(!beam[0]) break; //TODO - why this happens
    /*if(time(0) - start >= 120) {
      printf("time\n");
      break;
      }*/
    ...
    for(int i = 0; i < width; i++) {
      ...
      tnode *curr = beam[i];
      if(env->params->greer_params.classify_cuda)
	curr->data->res = classify(env, curr->data->v); //this repeats computation, but it is needed
      class_res res = curr->data->res;
      check_flip(env, curr);
