...
void set_sample_class(samples_t *samples, size_t class, int label,
                      size_t count);
...
/** Returns a new set of samples in which all negatives have been flipped into
 * positives, and in which there are no negative samples so all original
 * positives have been removed this is for the purpose of forcing all xs to be 1
 */
samples_t *flip_negatives(samples_t *);
 ...
 */
clusters_info_t *clusters_info_singleton(clusters_info_t *info, size_t count,
                                         size_t dimension);
...
typedef struct {
  ...
  /** Simplex side length */
  double side;
  /** Number of points in each simplex, or NULL for half of the positives to be
   * divided evenly among the clusters. */
  size_t *cluster_sizes;
  /** Scale parameter for exponentially distributed noise. 0 for sharp boundary
   */
  double scale;
  /** What fraction of the positives should be background noise? Value [0,1].
   * Default should be 0.5. TODO: get rid of this - it is redundant with
   * cluster_sizes */
  double noise_ratio;
} simplex_info_t;
...
/** Generates the positives in the cartesian product of a simplex in k dims with
[0,1]^(n-k) i.e. a prism
**/
samples_t *random_prism_samples(simplex_info_t *, size_t);

void mirror_sample(size_t, double *);

/** returns a hyperplane which cuts off the prism
 * if there are two, it cuts off the one by the origin
 **/
double *prism_cut_hplane(simplex_info_t *simplex_info, size_t simplex_dims);

// functions for generating samples where the positives follow some probability
// distribution that tightly clusters them around a corner so they will be like
...
typedef struct {
  ...
  /** Dimension of the sample space */
  size_t dimension;
  /** How many dimensions to sample according to dist? All others will be
   * uniform. */
  size_t dist_dims;
  ...
  cluster_dist dist;
  /** Desired radius of the cluster's feasible zone */
  double rad;
  /** Parameter(s) for the chosen distribution. Size depends on dist */
  double param;
  /** Norm exponent (1 or 2, to choose L1 or L2 norm) */
  int norm;
} fuzzy_info_t;

samples_t *random_fuzzy_samples(fuzzy_info_t *fuzzy_info);
double compute_param(size_t dimension, double global_ratio, double local_ratio,
                     double rad, cluster_dist dist);
/**  if fuzzy is generated wrt L1 norm, we can cut off the region where it
 * should be feasible */
...
typedef struct {
  ...
  double epsilon_prob;
  /** value of c for the exponential barrier */
  double c;
  ...
  struct greer_params {
    ...
    int beam_width;
    int classify_cuda; // 1 if we should use cuda for classifying (only in beam
                       // search for now)
    obj_code_t obj_code;
 ...
 * arguments */
void update_solution(solution_data_t *,
                     /** New solution vector */
                     double *solution,
...
/** log of solutions as time passes in the solve - expands automatically */
typedef struct {
  double *objs;
  double *times;
  size_t n_pts;
...
/** Allocate and return a solution log with init_cap intial capacity */
solution_log_t *solution_log_init(size_t init_cap);

/** Deallocates the elements in the log, but not the log itself. Returns a
 * pointer to the log */
solution_log_t *delete_solution_log(solution_log_t *log);

/** Append the given (obj, time) pair to the log, expanding as needed */
...
sparse_vector_t *precision_row(
    ...
    double theta);
sparse_vector_t *
precision_row_strict(samples_t *,
                     double); // the same, but without lagrangian relaxation
...
/** Generates a random vector in the prism given by the cartesian product of a
 * simplex with [0,1]^(n-k). Returns a newly allocated vector.
 */
...
double *single_siman_run(unsigned int *seed, int iter_lim, env_t *env,
                         double *h0);
...
typedef struct {
  ...
  int *need_recon; // at index D, equals 1 if w = y + au, or -1 if w = y, or 0
                   // if not yet considered, when reconstructing the solution to
                   // the D-dimensional subproblem. only for mixed
  struct last_proj {
    size_t class;
...
// to store the result of clasifying the dataset with a hyperplane, splitting
// samples into true positives, false positives, etc maybe it would be better to
// name these differently (based on "pos-pos" "pos-neg", "pos-zero", etc, so
// that the first word is always the class)
typedef struct class_res {
  int *tpos, *fpos, *tneg, *fneg, *pz, *nz;
  int ntpos, nfpos, ntneg, nfneg, npz, nnz;
  int nwrong, nright;
  double obj, max_poss_obj; // max_poss_obj = max possible objective (making
                            // pz's positive)
} class_res;
...
typedef struct hplane_data {
  ...
  unsigned int lists; // equals number of lists this appears in (to avoid
                      // freeing it with the tree)
  int res_ent;        // 1 if the ptrs in class_res are used by another data

  // TODO: maybe refactor to just keep track of the total number of references
  // to it
} hplane_data;

// each node must store the hyperplane found at that node, its classification
// result, the "most recent" sample which the hyperplane is orthogonal to, and
// neighbors maybe once a node's children have been created, we can delete the
// classification result (for memory) "most recent" means that you can traverse
// from a node to the root and get all of the samples to which the hyperplane is
// orthogonal the root node will have no loc
typedef struct tnode {
  ...
  int **branching_data; // branching_data[i][j] = 1 if sample (i,j) has been
                        // branched upon by an ancestor, 0 otherwise
  double score;
  int key;
  ...
  gsl_matrix *
      parent_basis; // num of cols = node.depth-1. may be NULL, depending on alg
} tnode;
...
typedef struct hplane_list {
  ...
  int max_possible_obj,
      min_possible_obj; // min_possible_obj is the minimum objective value,
                        // max_possible_obj is the maximum possible objective
                        // (including current zeros)
  int max_obj;
  int count;
  int poison; // equals 1 iff the list has been changed since last computing max
              // and min
} hplane_list;
...
void create_node_child(env_t *, tnode *, sample_locator_t, int **,
                       double score(env_t *, tnode *));
