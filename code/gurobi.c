...
GRBmodel *init_gurobi_model(int *state, const env_t *env) {
  ...
  TRY_MODEL_ON(GRBsetintattr(model, GRB_INT_ATTR_MODELSENSE, GRB_MAXIMIZE));
...
int add_gurobi_sample_var(GRBmodel *model, int label, char *name) {
  ...
  return 0;*/
  return GRBaddvar(model, 0, NULL, NULL, label_to_obj(label), 0., 1.,
                   GRB_BINARY, name);
  /*return GRBaddvar(model, 0, NULL, NULL,  //just for testing SVM-style
     relaxation label_to_obj(label), 0., GRB_INFINITY,  GRB_CONTINUOUS, name);*/
  /*return GRBaddvar(model, 0, NULL, NULL,  //removing reach term from obj
                   0,
                   0., 1.,  GRB_CONTINUOUS,
                   name);*/
}
...
int add_gurobi_sample_constr_bilinear(GRBmodel *model, sample_locator_t locator,
  ...
  sparse_vector_t *v =
      to_sparse(dimension, samples->samples[class][sample_index], 2);
  int col_idx = idx(0, class, sample_index, samples); // sample decision
                                                      // variable
  ...
  if (label > 0) {
    ...
    lval[0] = label > 0 ? -env->params->epsilon_positive
                        : env->params->epsilon_negative;
  } else {
  ...
  /*return GRBaddconstr(model,
                      ...
                      name);*/
  return GRBaddqconstr(model, lnz, lind, lval, v->len, v->ind + 1, qcol,
                       v->val + 1, GRB_GREATER_EQUAL, 0, name);
}

int add_gurobi_sample_constr_qp(GRBmodel *model, sample_locator_t locator,
                                int label, char *name, const env_t *env) {
  // Set coefficients of w
  samples_t *samples = env->samples;
  int dimension = (int)samples->dimension;
...
int add_gurobi_sample_constr_qp1(GRBmodel *model, sample_locator_t locator,
                                 int label, char *name, const env_t *env) {
  // Set coefficients of w
  samples_t *samples = env->samples;
  int dimension = (int)samples->dimension;
  ...
  sparse_vector_t *v = to_sparse(dimension, hplane_coefs, 2);
...
int add_gurobi_sample_constr_sep(GRBmodel *model, sample_locator_t locator,
                                 int label, char *name, const env_t *env) {
  // Set coefficients of w
  samples_t *samples = env->samples;
  int dimension = (int)samples->dimension;
  ...
  return GRBaddconstr(model, v->len, v->ind + 1, v->val + 1, GRB_GREATER_EQUAL,
                      1, name);
}

int add_gurobi_sample(GRBmodel *model, sample_locator_t locator,
                      const env_t *env) {
...
int add_gurobi_sample_bilinear(GRBmodel *model, sample_locator_t locator,
                               const env_t *env) {
  int label = env->samples->label[locator.class];
  char name[NAME_LEN_MAX];
  snprintf(name, NAME_LEN_MAX, "%c%u", label_to_varname(label),
...
int add_gurobi_sample_qp(GRBmodel *model, sample_locator_t locator,
                         const env_t *env) {
  int label = env->samples->label[locator.class];
  char name[NAME_LEN_MAX];
  snprintf(name, NAME_LEN_MAX, "%c%u", label_to_varname(label),
...
int add_gurobi_sample_sep(GRBmodel *model, sample_locator_t locator,
                          const env_t *env) {
  int label = env->samples->label[locator.class];
  char name[NAME_LEN_MAX];
  snprintf(name, NAME_LEN_MAX, "t%c%u", label_to_varname(label),
           (unsigned int)locator.index + 1);

  // Add sample decision variable
  TRY_STATE(GRBaddvar(model, 0, NULL, NULL, 1, 0, GRB_INFINITY, GRB_CONTINUOUS,
                      name););
...
int add_gurobi_sample_thr(GRBmodel *model, sample_locator_t locator,
                          const env_t *env) {
  int label = env->samples->label[locator.class];
  char name[NAME_LEN_MAX];
  snprintf(name, NAME_LEN_MAX, "%c%u", label_to_varname(label),
  ...
  // TODO: Threshold vars seem to cause an issue, since the index of the
  // variable V has been shifted by them being added could just move them into
  // the add-precision function for a hacky soln
...
int gurobi_accumulator(samples_t *samples, sample_locator_t locator,
                       void *model, void *env) {
  return add_gurobi_sample((GRBmodel *)model, locator, (const env_t *)env);
}

int gurobi_accumulator_bilinear(samples_t *samples, sample_locator_t locator,
                                void *model, void *env) {
  return add_gurobi_sample_bilinear((GRBmodel *)model, locator,
                                    (const env_t *)env);
}

int gurobi_accumulator_qp(samples_t *samples, sample_locator_t locator,
                          void *model, void *env) {
  return add_gurobi_sample_qp((GRBmodel *)model, locator, (const env_t *)env);
}

int gurobi_accumulator_sep(samples_t *samples, sample_locator_t locator,
                           void *model, void *env) {
  return add_gurobi_sample_sep((GRBmodel *)model, locator, (const env_t *)env);
}

int gurobi_accumulator_thr(samples_t *samples, sample_locator_t locator,
                           void *model, void *env) {
...
int add_gurobi_samples_qp(GRBmodel *model, const env_t *env) {
  return reduce(env->samples, (void *)model, gurobi_accumulator_qp,
                (void *)env);
}

int add_gurobi_samples_sep(GRBmodel *model, const env_t *env) {
  return reduce(env->samples, (void *)model, gurobi_accumulator_sep,
                (void *)env);
}

int add_gurobi_samples_thr(GRBmodel *model, const env_t *env) {
  return reduce(env->samples, (void *)model, gurobi_accumulator_thr,
                (void *)env);
}

int add_gurobi_samples_bilinear(GRBmodel *model, const env_t *env) {
  return reduce(env->samples, (void *)model, gurobi_accumulator_bilinear,
                (void *)env);
}

int add_gurobi_precision(GRBmodel *model, const env_t *env) {
  params_t *params = env->params;
  TRY_STATE(GRBaddvar(model, 0, NULL, NULL, -params->lambda,
                      params->violation_type ? 0. : 0, GRB_INFINITY,
                      GRB_CONTINUOUS, "V"));
...
int add_gurobi_precision_weak(GRBmodel *model, const env_t *env) {
  // want a weaker bound than V >= 0
  // we will make it like a leaky reLU
  // slope multiplier for when normal RHS is negative:
  // double alpha = 1/(env->params->lambda * env->params->theta);
  // double alpha = 1/(env->params->lambda * (2*env->params->theta - 1));
  // double alpha = 1e-8;
  double alpha = env->params->c;
  params_t *params = env->params;

  if (alpha == 0) {
    TRY_STATE(GRBaddvar(model, 0, NULL, NULL, -params->lambda, 0, GRB_INFINITY,
                        GRB_CONTINUOUS, "V"));
  } else {
    TRY_STATE(GRBaddvar(model, 0, NULL, NULL, -params->lambda, -GRB_INFINITY,
                        GRB_INFINITY, GRB_CONTINUOUS, "V"));
  }

  double theta = params->theta;
  sparse_vector_t *constraint = precision_row(env->samples, theta);
  gurobi_indices(constraint);
  TRY_STATE(GRBaddconstr(model, constraint->len, constraint->ind + 1,
                         constraint->val + 1, GRB_LESS_EQUAL,
                         -theta * params->epsilon_precision, "V"));

  if (alpha != 0) {
    constraint->val[constraint->len] /= alpha; // so we replace V with V/alpha
    return GRBaddconstr(model, constraint->len, constraint->ind + 1,
                        constraint->val + 1, GRB_LESS_EQUAL,
                        -theta * params->epsilon_precision, "V_lb");
  }
  return 0;
}

int add_gurobi_precision_lin_bar(GRBmodel *model, const env_t *env) {
  params_t *params = env->params;
  TRY_STATE(GRBaddvar(model, 0, NULL, NULL, -params->lambda, -GRB_INFINITY,
                      GRB_INFINITY, GRB_CONTINUOUS, "V"));
  ...
  // want a weaker bound than V >= 0
  // taking V' = max(V, aV + b(1-a))
  double alpha = 1 / (env->params->lambda * env->params->theta);
  double beta = -5000;
  // double alpha = 1;
  constraint->val[constraint->len] /= alpha; // so we replace V with V/alpha

  double rhs = -(theta * params->epsilon_precision + beta * (1. / alpha - 1));
  return GRBaddconstr(model, constraint->len, constraint->ind + 1,
                      constraint->val + 1, GRB_LESS_EQUAL, rhs, "V_lb");
}

int add_gurobi_precision_exp_bar(GRBmodel *model, const env_t *env) {
  samples_t *samples = env->samples;
  params_t *params = env->params;
  TRY_STATE(GRBaddvar(model, 0, NULL, NULL, 0, -GRB_INFINITY, GRB_INFINITY,
                      GRB_CONTINUOUS, "V"));
  ...
  gurobi_indices(constraint);
  TRY_STATE(GRBaddconstr(model, constraint->len, constraint->ind + 1,
                         constraint->val + 1, GRB_EQUAL,
                         -theta * params->epsilon_precision, "V"));

  // want a weaker bound than V >= 0
  // taking V' = c(e^(V/mu) - 1), although we will leave off the constant
  // it is mathematically nicer if c = mu, since then it has derivative 0 at V =
  // 0
  double c = env->params->c;
  double mu = c;
  // double alpha = 1;
  // constraint->val[constraint->len] /= alpha; //so we replace V with V/alpha
  TRY_STATE(GRBaddvar(model, 0, NULL, NULL, 0, -GRB_INFINITY, GRB_INFINITY,
  ...
  // E1 = V/mu <=> E1*mu - V = 0
  sparse_vector_t *e1 = sparse_vector_blank(3);
  int v_idx = violation_idx(0, samples);
  append(e1, v_idx, -1);
  append(e1, v_idx + 1, mu);
  gurobi_indices(e1);
  TRY_STATE(
      GRBaddconstr(model, 2, e1->ind + 1, e1->val + 1, GRB_EQUAL, 0, "E1"));
  free(delete_sparse_vector(e1));

  // E2 = e^E1
  TRY_STATE(GRBaddvar(model, 0, NULL, NULL, 0, -GRB_INFINITY, GRB_INFINITY,
                      GRB_CONTINUOUS, "E2"));
  TRY_STATE(GRBupdatemodel(model));
  TRY_STATE(GRBaddgenconstrExp(model, "exp", v_idx, v_idx + 1, ""));

  // p = V' = penalty = c(E2 - 1)
  TRY_STATE(GRBaddvar(model, 0, NULL, NULL, -params->lambda, -GRB_INFINITY,
                      GRB_INFINITY, GRB_CONTINUOUS, "p"));
  TRY_STATE(GRBupdatemodel(model));

  sparse_vector_t *p = sparse_vector_blank(2);
  append(p, v_idx + 2, env->params->c); // E2 with coeff c
  append(p, v_idx + 3, -1);             // p with coeff -1
  gurobi_indices(p);
  TRY_STATE(GRBaddconstr(model, 2, p->ind + 1, p->val + 1, GRB_EQUAL,
                         env->params->c, "p"));

  free(delete_sparse_vector(p));

  return 0;
}

int add_gurobi_precision_piecewise_exp_bar(GRBmodel *model, const env_t *env) {
  samples_t *samples = env->samples;
  params_t *params = env->params;
  TRY_STATE(GRBaddvar(model, 0, NULL, NULL, 0, -GRB_INFINITY, GRB_INFINITY,
                      GRB_CONTINUOUS, "V"));
  ...
  // need a variable SV equalling sign(V).
  // if we take L*SV <= V <= U*(1-SV), where L and U are bounds on V, that will
  // work. L can be |P|(theta-1), U can be |N|theta.
  // actually, that will make SV = 1-sign(V), which is fine
  double L = positives(env->samples) * (env->params->theta - 1);
  double U = negatives(env->samples) * env->params->theta;
  TRY_STATE(GRBaddvar(model, 0, NULL, NULL, 0, 0, 1, GRB_BINARY, "SV"));
  int u_idx = v_idx + 4;
  int ind[2];
  double val[2];
  ind[0] = v_idx - 1;
  ...
  val[0] = 1;
  val[1] = -L;
  TRY_STATE(GRBaddconstr(model, 2, ind, val, GRB_GREATER_EQUAL, 0, "SVL"));
  val[1] = U;
  TRY_STATE(GRBaddconstr(model, 2, ind, val, GRB_LESS_EQUAL, U, "SVU"));

  // if SV = 0, we want V' = V
  // if SV = 1, we want V' = p
  // can do it by saying V' >= V, and if SV = 0, then V' >= p (as in indicator
  // constr)
  TRY_STATE(GRBaddvar(model, 0, NULL, NULL, -params->lambda, -GRB_INFINITY,
                      GRB_INFINITY, GRB_CONTINUOUS, "Vp"));
  int vp_idx = v_idx + 5;
  int p_idx = v_idx + 3;
  // V' >= V:
  ind[0] = v_idx - 1;
  ...
  val[0] = 1;
  val[1] = -1;
  TRY_STATE(GRBaddconstr(model, 2, ind, val, GRB_LESS_EQUAL, 0, "VpGV"));
  // SV = 1 => V' >= p:
  ind[0] = vp_idx - 1;
  ind[1] = p_idx - 1;
  val[0] = 1;
  val[1] = -1;
  return GRBaddgenconstrIndicator(model, "pw", u_idx - 1, 1, 2, ind, val,
                                  GRB_GREATER_EQUAL, 0);
}
...
int add_gurobi_precision_strict(GRBmodel *model, const env_t *env) {
  ...
  /*TRY_STATE(
    GRBaddvar(model, 0, NULL, NULL,
              ...
              GRB_INFINITY,
              GRB_CONTINUOUS, "V"));*/
  TRY_STATE(GRBaddvar(model, 0, NULL, NULL, 0,
                      params->violation_type ? 0. : -GRB_INFINITY, GRB_INFINITY,
                      GRB_CONTINUOUS, "V"));
...
GRBmodel *gurobi_milp_tgm(int *state, const env_t *env) {
  samples_t *samples = env->samples;
  TRY_MODEL(!is_binary(samples))
  GRBmodel *model = init_gurobi_model(state, env);
  TRY_MODEL(NULL == model)
...
int add_gurobi_aux_qp_vars(GRBmodel *model, const env_t *env) {
  ...
  TRY_STATE(GRBaddvar(model, 0, NULL, NULL, 0, -GRB_INFINITY, 0, GRB_CONTINUOUS,
                      "L1"));
  TRY_STATE(GRBaddvar(model, 0, NULL, NULL, 0, -GRB_INFINITY, GRB_INFINITY,
                      GRB_CONTINUOUS, "Q1"));
  TRY_STATE(GRBaddvar(model, 0, NULL, NULL, 0, -GRB_INFINITY, GRB_INFINITY,
  ...
  append(cL1, iL1, -1);
  gurobi_indices(cL1);
  TRY_STATE(GRBaddconstr(model, cL1->len, cL1->ind + 1, cL1->val + 1, GRB_EQUAL,
                         0, "cL1"));

  // is it a memory leak to leave the sparse vector unfreed? the old functions
  // do it
  ...
  gurobi_indices(q1q);
  gurobi_indices(q1l);
  TRY_STATE(GRBaddqconstr(model, q1l->len, q1l->ind + 1, q1l->val + 1, q1q->len,
                          q1q->ind + 1, q1q->ind + 1, q1q->val + 1, GRB_EQUAL,
                          0, "cQ1"));
  ...
  gurobi_indices(q2q);
  gurobi_indices(q2l);
  return GRBaddqconstr(model, q2l->len, q2l->ind + 1, q2l->val + 1, q2q->len,
                       q2q->ind + 1, q2q->ind + 1, q2q->val + 1, GRB_EQUAL, 0,
                       "cQ2");
}
...
int add_gurobi_precision_qp(GRBmodel *model, const env_t *env) {
  ...
  gurobi_indices(q);
  int state =
      GRBaddqconstr(model, l->len, l->ind + 1, l->val + 1, q->len, q->ind + 1,
                    q->ind + 1, q->val + 1, GRB_LESS_EQUAL, 0, "qP");
...
int add_gurobi_unit_hplane(GRBmodel *model, const env_t *env) {
  ...
  return GRBaddqconstr(model, 0, NULL, NULL, q->len, q->ind+1, q->ind+1,
  q->val+1, GRB_EQUAL, 1, "w");*/
  ...
  return GRBaddqconstr(model, 0, NULL, NULL, q->len, q->ind+1, q->ind+1,
  q->val+1, GRB_LESS_EQUAL, 1, "w");*/

  TRY_STATE(GRBupdatemodel(model));

  TRY_STATE(
      GRBaddvar(model, 0, NULL, NULL, 0, 1, GRB_INFINITY, GRB_CONTINUOUS, "r"));

  int idx;
  TRY_STATE(GRBgetintattr(model, "NumVars", &idx));
  // getting the index of the last-added var, which is r
  // we would subtract 1 from it, except that we update the model before making
  // r, so it doesn't count toward the index yet
...
int gurobi_cones_miqp_accumulator(samples_t *samples, sample_locator_t locator,
                                  void *modela, void *enva) {
...
int add_gurobi_cones(GRBmodel *model, const env_t *env) {
  ...
  // add binary vars vs:
  TRY_STATE(reduce(samples, (void *)model, gurobi_cones_miqp_accumulator,
                   (void *)env));
  ...
  gurobi_indices(cv);
  TRY_STATE(GRBaddconstr(
      model, cv->len, cv->ind + 1, cv->val + 1, GRB_GREATER_EQUAL, n - (d - 1),
      "cv")); // could also be EQUAL to only explore leaf nodes
  TRY_STATE(GRBaddconstr(model, cv->len, cv->ind + 1, cv->val + 1,
                         GRB_LESS_EQUAL, n - 1,
                         "cv")); // this way, it must be in the tree
  ...
  for (int i = 0; i < n - 1; i++) {
    ...
    snprintf(name1, NAME_LEN_MAX, "u%c%u", label_to_varname(label),
             (unsigned int)loc->index + 1);
    snprintf(name2, NAME_LEN_MAX, "l%c%u", label_to_varname(label),
             (unsigned int)loc->index + 1);

    // Tight bounds:
    TRY_STATE(GRBaddconstr(model, l1->len, l1->ind + 1, l1->val + 1,
                           GRB_LESS_EQUAL, 0, name1));
    TRY_STATE(GRBaddconstr(model, l2->len, l2->ind + 1, l2->val + 1,
                           GRB_GREATER_EQUAL, 0, name2));

    /*//with leeway:
    double eps = 0.03;

    TRY_STATE(GRBaddconstr(model, l1->len, l1->ind+1, l1->val+1, GRB_LESS_EQUAL,
    eps, name1)); TRY_STATE(GRBaddconstr(model, l2->len, l2->ind+1, l2->val+1,
    GRB_GREATER_EQUAL, -eps, name2));*/
  ...
  for(int i = 0; i < n-1; i++) {
    ...
    sparse_vector_t *qcol = sparse_vector_blank(d); //values will be stored in
  here for(int j = 0; j < d; j++) { append(qrow, vi, 1); append(qcol, j,
  samples->samples[loc->class][loc->index][j]);
    }
    gurobi_indices(qrow);
    ...
    TRY_STATE(GRBaddqconstr(model, 0, NULL, NULL, qrow->len, qrow->ind+1,
  qcol->ind+1, qcol->val+1, GRB_EQUAL, 0, name));
...
int gurobi_constrain_side(GRBmodel *model, const env_t *env,
                          sample_locator_t loc, int side) {
  // constrains sample to be on the side of the hyperplane given by side (=
  // +/-1)
  samples_t *samples = env->samples;
  int dimension = (int)samples->dimension;
  ...
  char name[NAME_LEN_MAX];
  snprintf(name, NAME_LEN_MAX, "%c%c%u", side == 1 ? 'p' : 'n',
           label_to_varname(env->samples->label[loc.class]),
           (unsigned int)loc.index + 1);

  if (side == 1) {
    return GRBaddconstr(model, v->len, v->ind + 1, v->val + 1,
                        GRB_GREATER_EQUAL, env->params->epsilon_positive, name);
  } else {
    return GRBaddconstr(model, v->len, v->ind + 1, v->val + 1, GRB_LESS_EQUAL,
                        -env->params->epsilon_negative, name);
  }
}

int add_gurobi_small_cone_constr(GRBmodel *model, const env_t *env, double *w,
...
int add_gurobi_small_cone_constrs(GRBmodel *model, const env_t *env,
                                  double *w) {
  samples_t *samples = env->samples;
  int state = 0;
  /*printf("hyperplane: ");
...
GRBmodel *gurobi_relax_within_small_cone(int *state, const env_t *env,
                                         double *w) {
  samples_t *samples = env->samples;
  TRY_MODEL(!is_binary(samples));
  GRBmodel *model = init_gurobi_model(state, env);
...
int add_gurobi_ortho(GRBmodel *model, const env_t *env, sample_locator_t loc) {
  ...
  return GRBaddconstr(model, v->len, v->ind + 1, v->val + 1, GRB_EQUAL, 0,
                      name);
}

int gurobi_add_subspace_constraints(GRBmodel *model, const env_t *env,
                                    sample_locator_t *basis, int ss_dim) {
  for (int i = 0; i < ss_dim; i++) {
    TRY_STATE(add_gurobi_ortho(model, env, basis[i]));
  }

  return GRBupdatemodel(model);
}

int gurobi_remove_last_k_constraints(GRBmodel *model, int k) {
  int nconstrs;
  TRY_STATE(GRBgetintattr(model, "NumConstrs", &nconstrs));
  int *ind = CALLOC(k, int);
  for (int i = 0; i < k; i++) {
    ind[i] = nconstrs - k + i;
  }

  return GRBdelconstrs(model, k, ind);
}

GRBmodel *gurobi_relax_within_subspace(int *state, const env_t *env,
                                       sample_locator_t *basis, int ss_dim) {
  samples_t *samples = env->samples;
  TRY_MODEL(!is_binary(samples));
  GRBmodel *model = init_gurobi_model(state, env);
  ...
  if (ss_dim > 0)
    TRY_MODEL_ON(gurobi_add_subspace_constraints(model, env, basis, ss_dim));
...
int add_gurobi_outside_cone_vars(GRBmodel *model, const env_t *env, int *cone,
                                 int n_supp, int cone_idx) {
  char *vtypes = CALLOC(n_supp, char);
  char **names = CALLOC(n_supp, char *);
  for (int i = 0; i < n_supp; i++) {
    vtypes[i] = GRB_BINARY;
    char name[NAME_LEN_MAX];
    snprintf(name, NAME_LEN_MAX, "z%u_%u", cone_idx,
             i); // has a slightly misleading name - maybe i should be replaced
                 // by a sample idx
    names[i] = name;
  }

  int res = GRBaddvars(model, n_supp, 0, NULL, NULL, NULL, NULL, NULL, NULL,
                       vtypes, names);
...
int add_gurobi_outside_cone(GRBmodel *model, const env_t *env, int *cone,
                            int cone_idx) {
  samples_t *samples = env->samples;
  ...
  // TRY_STATE(add_gurobi_outside_cone_vars(model, env, cone, n_supp,
  // cone_idx));
  ...
  int res =
      GRBaddconstr(model, n_supp, ind, val, GRB_LESS_EQUAL, n_pos - 1, name);
  free(ind);
  free(val);
  return res;
}

int add_gurobi_outside_cone_lazy(void *cbdata, const env_t *env, int *cone,
                                 int cone_idx) {
  samples_t *samples = env->samples;
  int n = samples_total(samples);
  int d = samples->dimension;
...
int add_gurobi_one_reach(GRBmodel *model, const env_t *env) {
  ...
  // may lead to infeasibility, although this is actually good for the cone exp
  // search

  int idx_min =
      idx_extreme(0, 1, 0, env->samples); // first index of positive samples
...
int add_gurobi_samples_forced(GRBmodel *model, const env_t *env) {
  // adds samples with constraints s.t. xs and ys will always be consistent with
  // the hyperplane, regardless of objective may be missing epsilons
  ...
  for (int i = 0; i < P; i++) {
    ...
    TRY_STATE(GRBaddconstr(model, v->len, v->ind + 1, v->val + 1,
                           GRB_LESS_EQUAL, -env->params->epsilon_positive,
                           name));
  ...
  for (int i = 0; i < N; i++) {
    ...
    TRY_STATE(GRBaddconstr(model, v->len, v->ind + 1, v->val + 1,
                           GRB_GREATER_EQUAL, env->params->epsilon_negative - 1,
                           name));
    free(delete_sparse_vector(v));
  }
  return 0;
...
GRBmodel *gurobi_find_outside_cones(int *state, const env_t *env, int **cones,
                                    int n_cones) {
  samples_t *samples = env->samples;
  TRY_MODEL(!is_binary(samples));
  GRBmodel *model = init_gurobi_model(state, env);
...
GRBmodel *gurobi_milp_strict(int *state, const env_t *env) {
  // like gurobi_milp, but forces xs and ys to be consistent with w, no
  // lagrangian relaxation, and forces ||w|| <= 1
  samples_t *samples = env->samples;
  TRY_MODEL(!is_binary(samples));
  GRBmodel *model = init_gurobi_model(state, env);
  TRY_MODEL(NULL == model);
...
double insep_score(env_t *env) {
  ...
  TRY_STATE(GRBsetintattr(model, GRB_INT_ATTR_MODELSENSE, GRB_MINIMIZE));
  ...
  for (size_t class = 0; class < samples->class_cnt; class ++) {
    for (size_t i = 0; i < samples->count[class]; i++) {
      ...
      for (size_t j = 0; j < samples->count[class]; j++) {
        if (j == i)
          continue;
        double dist = euclid_dist(samples->samples[class][i],
                                  samples->samples[class][j], d);
        if (dist < min_inter)
          min_inter = dist;
      }
